...
class TydomMessageHandler:
        ...
        if self.mqtt_client is None:
            ...
            try:
                if "Uri-Origin: /refresh/all" in first in first:
                    ...
                    _LOGGER.debug("PUT /devices/data message detected !")
                    try:
                        try:
                        ...
                        _LOGGER.error(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
                        _LOGGER.error("RAW INCOMING :")
                        _LOGGER.error(bytes_str)
                    ...
                    try:
                        ...
                        await self.parse_response(incoming)
                        _LOGGER.info("Scenarii message processed !")
                        _LOGGER.info("##################################")
                    except BaseException:
                        _LOGGER.error(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
                        _LOGGER.error("RAW INCOMING :")
                    ...
                    try:
                        incoming = self.parse_put_response(bytes_str)
                        await self.parse_response(incoming)
                        _LOGGER.info("POST message processed !")
                    except BaseException:
                        _LOGGER.error(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
                        _LOGGER.error("RAW INCOMING :")
                        _LOGGER.error(bytes_str)
                    ...
                    _LOGGER.warn("Didn't detect incoming type, here it is :")
                    _LOGGER.warn(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
                    _LOGGER.warn("RAW INCOMING :")
                    _LOGGER.warn(bytes_str)
                    _LOGGER.warn("END RAW")
                    _LOGGER.warn(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")

            except Exception as e:
                _LOGGER.error("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                _LOGGER.error("receiveMessage error")
                _LOGGER.error("RAW :")
                _LOGGER.error(bytes_str)
                _LOGGER.error("Incoming payload :")
                _LOGGER.error(incoming)
                _LOGGER.error("Error :")
                _LOGGER.error(e)
                _LOGGER.error("Exiting to ensure systemd restart....")
    ...
    async def parse_response(self, incoming):
        ...
        if data != "":
            ...
            if "id_catalog" in data:
                _LOGGER.debug(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
                _LOGGER.debug("Incoming message type : config detected")
                msg_type = "msg_config"
                _LOGGER.debug(data)
            elif "cmetadata" in data:
                _LOGGER.debug(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
                _LOGGER.debug("Incoming message type : cmetadata detected")
                msg_type = "msg_cmetadata"
                _LOGGER.debug(data)
            elif "cdata" in data:
                _LOGGER.debug(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
                _LOGGER.debug("Incoming message type : cdata detected")
                msg_type = "msg_cdata"
                _LOGGER.debug(data)
            elif "id" in first:
                _LOGGER.debug(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
                _LOGGER.debug("Incoming message type : data detected")
                msg_type = "msg_data"
                _LOGGER.debug(data)
            elif "doctype" in first:
                _LOGGER.debug("Incoming message type : html detected (probable 404)")
                msg_type = "msg_html"
                _LOGGER.debug(data)
            elif "productName" in first:
                _LOGGER.debug(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
                _LOGGER.debug("Incoming message type : Info detected")
                msg_type = "msg_info"
                _LOGGER.debug(data)
            else:
                _LOGGER.debug("Incoming message type : no type detected")
                _LOGGER.debug(data)
            ...
            if not (msg_type is None):
                try:
                    if msg_type == "msg_config":
                        ...
                        _LOGGER.debug(
                            json.dumps(
                                parsed, sort_keys=True, indent=4, separators=(",", ": ")
                            )
                        )
                except Exception as e:
                    _LOGGER.error("Cannot parse response !")
                    # _LOGGER.error('Response :')
                    # _LOGGER.error(data)
                    if e != "Expecting value: line 1 column 1 (char 0)":
                        _LOGGER.error("Error : ", e)
                        _LOGGER.error(parsed)
            _LOGGER.info("Incoming data parsed successfully !")
            return 0
    ...
    async def parse_config_data(self, parsed):
        for i in parsed["endpoints"]:
            ...
            if (
                ...
                or i["last_usage"] == "boiler"
                or i["last_usage"] == "conso"
                or i["last_usage"] == "interrupter"
                or i["last_usage"] == "plug"
            ):
                # _LOGGER.debug('%s %s'.format(i["id_endpoint"],i["name"]))
    ...
    async def parse_cmeta_data(self, parsed):
                            ...
                            for params in elem["parameters"]:
                                if params["name"] == "dest":
                                    for dest in params["enum_values"]:
                                        ...
                                        _LOGGER.debug("Add poll device : " + url)
        ...
        _LOGGER.info("Metadata configuration updated")
                ...
                if endpoint["error"] == 0 and len(endpoint["data"]) > 0:
                    ...
                    if device_data["validity"] == "upToDate":
                        ...
                        # Interrupter
                            ...
                            attr["payload_on"] = "TOGGLE"
                            attr["payload_off"] = "IDLE"
                        ...
                        if attr["type"] == "sensor":
                            ...
                            device = Switch_2(attr, self.mqtt_client)

                        elif attr["type"] == "binary_sensor":
                            device = Binary_Sensor_2(attr, self.mqtt_client)

                        elif attr["type"] == "light":
                            device = Light_2(attr, self.mqtt_client)

                        elif attr["type"] == "button":
                            device = Button_2(attr, self.mqtt_client)

                        if device != None:
                            _LOGGER.debug(
                                "PARSE DATA 2 : DEVICE UPDATE {}".format(attr["type"])
                            )
                            await device.update()
                            _LOGGER.debug("PARSE DATA 2 : DEVICE UPDATE OK")
                ...
                if endpoint["error"] == 0 and len(endpoint["data"]) > 0:
                    try:
                        ...
                        for elem in endpoint["data"]:
                            ...
                            if type_of_id == "light":
                                if (
                                    elementName in deviceLightKeywords
                                    and elementValidity == "upToDate"
                                ):  # NEW METHOD
                            ...
                            if type_of_id == "shutter" or type_of_id == "klineShutter":
                                if (
                                    elementName in deviceCoverKeywords
                                    and elementValidity == "upToDate"
                                ):  # NEW METHOD
                            ...
                            if type_of_id == "belmDoor" or type_of_id == "klineDoor":
                                if (
                                    elementName in deviceDoorKeywords
                                    and elementValidity == "upToDate"
                                ):  # NEW METHOD
                            ...
                            if type_of_id == "boiler":
                                if (
                                    elementName in deviceBoilerKeywords
                                    and elementValidity == "upToDate"
                                ):  # NEW METHOD
                            ...
                            if type_of_id == "alarm":
                                if (
                                    elementName in deviceAlarmKeywords
                                    and elementValidity == "upToDate"
                                ):
                            ...
                            if type_of_id == "garage_door" or type_of_id == "gate":
                                if (
                                    elementName in deviceSwitchKeywords
                                    and elementValidity == "upToDate"
                                ):  # NEW METHOD
                            ...
                            if type_of_id == "conso":
                                if (
                                    elementName in device_conso_keywords
                                    and elementValidity == "upToDate"
                                ):
                                    attr_conso = {
                                        "device_id": device_id,
                                    ...
                                    if elementName in device_conso_classes:
                                        attr_conso[
                                            "device_class"
                                        ] = device_conso_classes[elementName]
                            ...
                            if type_of_id == "unknown":
                                if (
                                    elementName in deviceMotionKeywords
                                    and elementValidity == "upToDate"
                                ):  # NEW METHOD
                        ...
                        _LOGGER.error("msg_data error in parsing !")
                        _LOGGER.error(e)
                    ...
                    # Get last known state (for alarm) # NEW METHOD
                        ...
                        try:
                            ...
                            if not (state is None):
                                ...
                                # _LOGGER.debug("Alarm created / updated : "+alarm)
                                alarm = Alarm(
                                    current_state=state,
                            ...
                            _LOGGER.error("Error in alarm parsing !")
                            _LOGGER.error(e)
                            pass
                    else:
                        pass
            ...
            for endpoint in i["endpoints"]:
                if endpoint["error"] == 0 and len(endpoint["cdata"]) > 0:
                    try:
                        ...
                        _LOGGER.error("msg_cdata error in parsing !")
                        _LOGGER.error(e)
    ...
    def get_type_from_id(self, id):
        ...
        if len(device_type) != 0 and id in device_type.keys():
            ...
            _LOGGER.warn("%s not in dic device_type", id)
    ...
    def get_name_from_id(self, id):
        ...
        if len(device_name) != 0 and id in device_name.keys():
            ...
            _LOGGER.warn("%s not in dic device_name", id)
        return name
