...
class Light_2:
    def __init__(self, attr, mqtt=None):
        ...
        self.light_config_topic = light_config_topic.format(
            id=self.attr["device_id"], endpoint_id=self.attr["endpoint_id"]
        )
        self.light_state_topic = light_state_topic.format(
            id=self.attr["device_id"], endpoint_id=self.attr["endpoint_id"]
        )
        self.light_attributes_topic = light_attributes_topic.format(
            id=self.attr["device_id"], endpoint_id=self.attr["endpoint_id"]
        )
        self.light_command_topic = light_command_topic.format(
